...
const transactionalMails = require('@confere-stores/transactional-mails')
...
const unsubscribeUri = 'https://mail-store.confere.com.br/aws/unsubscribe-mail/'
      ...
      setTimeout(async () => {
        ...
        Promise
          ...
          .then(({ info, options }) => {
            ...
            logger.log(`>> Email (${options.to},${options.cc}) - ${(new Date()).toISOString()}: #${storeId} ${statusRecordId}`)
          ...
          .catch(err => {
            if (err.type !== 'IgnoreTrigger' && err.code !== 'InvalidParameterValue') {
              ...
              if (!isRetry && (!err.response || !(err.response.status >= 400 && err.response.status < 500))) {
                ...
                errorHandling(err)
                console.error(err)
                logger.log(`>> Notification failed for #${storeId} ${orderId} - ${(new Date()).toISOString()}`)
              }
            }
          })
